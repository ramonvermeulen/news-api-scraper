...
class Mailer:
    def __init__(self, sender_mail, sender_password, smtp_server_address='smtp.gmail.com', smtp_port=587, recipients=[]):
    ...
    def send_mail(self, subject, body):
        ...
        try:
            body = '\r\n'.join(['To: %s' % self.recipients,
                                'From: %s' % self.sender_mail,
                                'Subject: %s' % subject,
                                '', body])
            server.sendmail(self.sender_mail, self.recipients, body)
            logging.info(f'[{datetime.datetime.now()}] - Email send to {self.recipients}')
